...
import QuestionScreen2 from './presenter';
...
class Container extends React.Component {
    ...
    init = async( ) => {
        try{
            let response = await fetch('http://주소/FetchMailCard', {
                method: 'POST',
                headers: {
            ...
            // ↓ 답변이 존재하지 않는 경우
            if(json.length === 0) {
                await this.setState({currentMailID: json.slice(-1)[0].mailID}); // → render_2
    ...
    _onEndReached = async() => {
            let response = await fetch('http://주소/FetchMailCard2', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    ...
                    userode: this.props.userProfile.userCode //fetchUserCode 한 내용을 넣자
                })
            });
            var json = await response.json();
